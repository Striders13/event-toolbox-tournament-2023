...
/obj/machinery/computer/tournament_controller/proc/spawn_toolboxes(toolbox_color, team_spawn_id, number_to_spawn)
	...
	for (var/_ in 1 to number_to_spawn)
		...
		var/obj/item/clothing/under/color/white/suit = new
		suit.color = toolbox_color
		var/obj/item/clothing/shoes/sneakers/white/shoes = new
		shoes.color = toolbox_color
		suit.forceMove(get_turf(spawn_landmark))
		shoes.forceMove(get_turf(spawn_landmark))
		toolbox.forceMove(get_turf(spawn_landmark))
